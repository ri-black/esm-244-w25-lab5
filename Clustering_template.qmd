...
```{r}
seeds_df <- read_tsv(here::here('data','seeds_dataset.txt'))  ### sometimes it is better to lazy load 'here::here' so that you don't bring all the other data/commands into the memory of R if we don't need that package
...
- scientists had manually put '-999' in as placeholders for NA values
...
seeds_df <- read_tsv(here::here('data','seeds_dataset.txt'),
                     col_names = FALSE,
                     na = '-999') %>%         # can also use case_when() in mutate() to do this
  setNames(var_names) %>%
...
number_est <- NbClust(seeds_scale, min.nc = 2, max.nc = 10, method = "kmeans")    
              # how many clusters do we think are within the range of the data
...
# nstart=x  reiterates starting place of clustering x amount of times
seeds_km 
```
...
```{r}
#| tbl-cap: Contiency table for the grouping of kmeans and the varieties of kernals.
...
- sort by (slice()) the top 20 emitters
- drop na (note: none in thios data)
- select only the numeric values
- scale the data
- get the distance if the scaled data
- run hclust()
- linkages, either single or complete
- make out dendrograms
...
# this function names the rows with the origional region name while only keeping scaled numeric data in the df
